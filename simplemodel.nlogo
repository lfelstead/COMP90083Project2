...
patches-own [ path? popularity]
tourists-own [ goal ]

;; reads in environment using txt file
;; paths (in gray) do not degrade and tourists want to keep to paths
;; attractions are placed with mouse click, tourists want to visit attractions
;; vegetation degrades when stepped on 

to setup
  clear-all
  set path-file "straightpath.txt"
  set-default-shape attractions "house"
  set-default-shape tourists "person"
  load-patch-data
  create-tourists tourist-count [
  ...
  ifelse mouse-down? [
    if not mouse-clicked? [
      ...
      ask patch mouse-xcor mouse-ycor [
        sprout-attractions 1 [set color red set size 2]
      ]
    ]
  ] [
...
to vetegation-growth
  ask patches with [ not any? tourists-here ] [
    set popularity popularity * (100 - vegetation-growth-rate) / 100
  ]
end

to become-more-popular
  set popularity popularity + damage-per-step
end

to recolor-patches
  ask patches with [ not path? ] [
    let max-value (80 * 3)
    set pcolor scale-color green popularity (- max-value) max-value
    if popularity >= 80 [ set pcolor brown ]
   ]
end
